...
>(({ className, children, title, ...props }, ref) => {
  const hasDialogTitle = (nodes: React.ReactNode): boolean => {
    ...
    for (const node of arr) {
      ...
      if (type === DialogPrimitive.Title || type?.displayName === DialogPrimitive.Title.displayName) {
        return true;
      }
      if (node.props && node.props.children) {
        if (hasDialogTitle(node.props.children)) return true;
      }
    }
    return false;
  };

  const ariaTitle = (props as any)["aria-label"] ?? title ?? "Dialog";
